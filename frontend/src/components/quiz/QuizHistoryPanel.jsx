...
const QuizHistoryPanel = ({ attempts, isLoading, onViewAttempt }) => {
  ...
  if (attempts.length === 0) {
    return (
      <div className="space-y-3">
        <div className="text-center py-20 bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm">
          <History size={48} className="mx-auto mb-4 text-slate-200 dark:text-slate-700" />
          <p className="font-bold text-slate-400 dark:text-slate-500 text-sm">아직 풀이 기록이 없습니다.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-3">
      {attempts.map(attempt => (
        <div
          ...
          className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm hover:border-blue-200 dark:hover:border-blue-500/40 hover:shadow-md transition-all cursor-pointer flex items-center justify-between group"
        >
          <div className="flex items-center gap-4">
            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black text-sm ${attempt.score === attempt.totalQuestions ? 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' : 'bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400'}`}>
              {Math.round((attempt.score / attempt.totalQuestions) * 100)}%
            </div>
            <div>
              <h3 className="font-bold text-slate-900 dark:text-slate-100 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{attempt.quizTitle}</h3>
